...
// Shared input style — uses pa-* tokens so it flips with data-theme
const inputCls =
...
function Select({
  ...
  return (
    <select
      ...
      style={{ ...inputStyle, color: value ? "var(--pa-ink)" : "var(--pa-ink-2)" }}
      onFocus={onFocus}
      onBlur={onBlur}
    >
      <option value="" disabled style={{ color: "var(--pa-ink-2)", background: "var(--pa-surface)" }}>{placeholder}</option>
      {options.map((o) => (
        <option key={o.value} value={o.value} style={{ color: "var(--pa-ink)", background: "var(--pa-surface)" }}>{o.label}</option>
      ))}
    </select>
  );
}
          ...
          >
            ...
            <div className="px-6 pt-5 pb-4 flex items-start justify-between gap-4">
              <div>
                ...
                <p className="text-[13px] mt-1" style={{ color: "var(--pa-ink-2)" }}>
                  See {entityName} in action — live, for your print shop.
                </p>
              </div>
              <button
                onClick={onClose}
            ...
            {status === "success" ? (
              <div className="px-6 pb-8 flex flex-col items-center text-center">
                ...
                <h3 className="text-lg font-bold mb-2" style={{ color: "var(--pa-ink)" }}>Demo Booked!</h3>
              ...
              <form onSubmit={handleSubmit} className="px-6 pb-6 space-y-3.5">
                <div className="grid grid-cols-2 gap-3">
                  <Field label="First Name" required>
                    <Input autoRef={firstRef} value={form.firstName} onChange={set("firstName")} placeholder="Sarah" required />
                  </Field>
