...
export default function HeroSection() {
  return (
    <>
      ...
      {/*
        KEY FIX:
        - section is NOT flex-col anymore
        - min-h-screen with pt-[70px] (navbar height) guarantees content
          ALWAYS starts below navbar regardless of scroll/refresh state
        - content is centered via its own flex column with flex-1
      */}
      <section
        id="home"
        className="relative min-h-screen bg-[#0a0b14] overflow-hidden"
      >
        {/* Glow blobs */}
        ...
        {/*
          pt-[70px] = navbar height — content never goes under navbar
          The inner div uses flex + min-h to vertically center the content
          in the remaining space below the navbar
        */}
        <div className="relative z-10 pt-[70px] min-h-screen flex flex-col">
          <div className="flex-1 flex flex-col items-center justify-center text-center px-4 py-16 w-full max-w-[760px] mx-auto">
            ...
            <div className="fu-5 flex items-center gap-3">
              ...
              {socials.map(({ label, href, icon }) => (
                <a
                  ...
                  href={href}
                  aria-label={label}
                  className="social-icon w-10 h-10 rounded-full border border-white/[0.15] bg-white/[0.05] flex items-center justify-center text-gray-400"
                >
                  {icon}
                </a>
              ))}
        ...
        {/* Scroll to explore — absolute at bottom of section */}
        <div className="absolute bottom-7 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 pointer-events-none select-none z-10">
          <span className="scroll-label text-[10.5px] tracking-[0.2em] uppercase font-medium text-gray-500">
            Scroll to explore
          </span>
          <svg
            className="arrow-float text-gray-600"
