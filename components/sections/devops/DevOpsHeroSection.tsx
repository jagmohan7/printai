...
const TILE_POSITIONS = [
  { width: 88,  height: 88,  top: 44,  left: 22,  anim: "ftA 5.5s ease-in-out infinite",        radius: 20, iconSize: 28, glow: "g-teal",   color: "var(--pa-teal)", border: "border-[var(--pa-line)]", bg: "bg-[var(--pa-card)]", zIndex: 1 },
  { width: 88,  height: 88,  top: 44,  left: 350, anim: "ftB 6s ease-in-out .5s infinite",      radius: 20, iconSize: 28, glow: "g-teal",   color: "var(--pa-teal)", border: "border-[var(--pa-line)]", bg: "bg-[var(--pa-card)]", zIndex: 1 },
  { width: 74,  height: 74,  top: 106, left: 193, anim: "ftZ 4.8s ease-in-out 1s infinite",     radius: 16, iconSize: 22, glow: "g-teal",   color: "var(--pa-teal)", border: "border-[var(--pa-line)]", bg: "bg-[var(--pa-card)]", zIndex: 15 },
  { width: 120, height: 120, top: 170, left: 170, anim: "ftC 5s ease-in-out 1.2s infinite",     radius: 24, iconSize: 38, glow: "g-teal",   color: "var(--pa-teal)", border: "border-2 border-[color:var(--pa-teal)]", bg: "bg-[var(--pa-card)]", zIndex: 20, isCenter: true },
  { width: 88,  height: 88,  top: 316, left: 22,  anim: "ftD 5.8s ease-in-out .8s infinite",    radius: 20, iconSize: 28, glow: "g-teal-d", color: "var(--pa-teal-deep)", border: "border-[var(--pa-line)]", bg: "bg-[var(--pa-card)]", zIndex: 1 },
  { width: 88,  height: 88,  top: 316, left: 350, anim: "ftA 6.2s ease-in-out 1.5s infinite",   radius: 20, iconSize: 28, glow: "g-teal",   color: "var(--pa-teal)", border: "border-[var(--pa-line)]", bg: "bg-[var(--pa-card)]", zIndex: 1 },
];
    ...
    <span key={key}>
      {parts.map((p, i) =>
        p.startsWith("**") && p.endsWith("**") ? (
          <span key={i} style={{ color: "var(--pa-teal)" }}>
            {p.slice(2, -2)}
          </span>
        ) : (
          <span key={i}>{p}</span>
    ...
    <section className="relative min-h-[92vh] flex items-center overflow-hidden pa-band-page px-4 section-hero">
      <style>{`
        ...
        @keyframes borderPulse {
          0%,100% { box-shadow: 0 0 18px rgba(103,61,230,.45), 0 0 40px rgba(103,61,230,.2), inset 0 0 12px rgba(103,61,230,.08); }
          50% { box-shadow: 0 0 28px rgba(103,61,230,.7), 0 0 60px rgba(103,61,230,.35), inset 0 0 20px rgba(103,61,230,.12); }
        }
        .center-tile { animation: borderPulse 2.4s ease-in-out infinite; }
        .g-teal { filter: drop-shadow(0 0 5px rgba(103,61,230,.9)) drop-shadow(0 0 12px rgba(103,61,230,.55)); }
        .g-teal-d { filter: drop-shadow(0 0 5px rgba(80,37,209,.9)) drop-shadow(0 0 12px rgba(80,37,209,.55)); }
        .dev-tile { transition: box-shadow .28s, transform .28s, border-color .28s; }
        .dev-tile:hover { transform: scale(1.07) translateY(-3px); box-shadow: 0 0 24px rgba(103,61,230,.22); border-color: rgba(103,61,230,.4) !important; }
      `}</style>

      <div className="pointer-events-none absolute inset-0">
        <div className="absolute top-[15%] right-[10%] w-[480px] h-[480px] rounded-full blur-[130px]" style={{ background: "rgba(103,61,230,0.1)" }} />
        <div className="absolute bottom-[10%] left-[5%] w-[400px] h-[400px] rounded-full blur-[130px]" style={{ background: "rgba(103,61,230,0.08)" }} />
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        <div>
          <MotionInView>
            <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-[12px] font-semibold tracking-widest uppercase mb-6" style={{ borderColor: "rgba(103,61,230,0.3)", border: "1px solid rgba(103,61,230,0.3)", background: "rgba(103,61,230,0.1)", color: "var(--pa-teal)" }}>
              {badge}
            </span>
          </MotionInView>
          ...
          <MotionInView delay={0.4}>
            <div className="mt-10 flex flex-wrap gap-4">
              ...
              <Link href={ctaSecondaryHref} className="inline-flex items-center gap-2.5 px-8 py-[15px] rounded-xl font-semibold text-[15px] pa-ink-text border border-[color:var(--pa-line)] hover:border-[color:var(--pa-teal)] hover:-translate-y-0.5 transition-all duration-200" style={{ background: "transparent" }}>
                {ctaSecondaryText}
              </Link>
            </div>
          </MotionInView>
        ...
        <MotionInView delay={0.2} className="hidden lg:flex items-center justify-center">
          <div className="relative" style={{ width: 460, height: 420 }}>
            <div className="pointer-events-none absolute rounded-full blur-[80px]" style={{ width: 300, height: 300, top: 60, left: 80, background: "rgba(103,61,230,0.12)" }} />
            ...
            <svg className="pointer-events-none absolute inset-0 w-full h-full" style={{ zIndex: 0 }} xmlns="http://www.w3.org/2000/svg">
              ...
              <path d="M -20,310 C 20,270 55,195 110,220 C 165,245 168,232 230,228 C 292,224 330,265 385,215 C 425,175 455,205 490,188" fill="none" stroke="rgba(103,61,230,0.5)" strokeWidth="1.3" strokeOpacity="0.14" />
              <path className="arc-comet" d="M -20,310 C 20,270 55,195 110,220 C 165,245 168,232 230,228 C 292,224 330,265 385,215 C 425,175 455,205 490,188" fill="none" stroke="var(--pa-teal)" strokeWidth="2.2" strokeLinecap="round" filter="url(#arcGlow)" />
              <path className="arc-comet-2" d="M -20,310 C 20,270 55,195 110,220 C 165,245 168,232 230,228 C 292,224 330,265 385,215 C 425,175 455,205 490,188" fill="none" stroke="var(--pa-teal-deep)" strokeWidth="1.6" strokeLinecap="round" strokeOpacity="0.55" filter="url(#arcGlow)" />
            </svg>
            ...
            {iconCluster.slice(0, 6).map((tile, i) => {
              ...
              return (
                <div
                  ...
                  title={tile.label}
                >
                  <Icon size={p.iconSize} className={p.glow} strokeWidth={1.4} style={{ color: p.color }} />
                </div>
              );
            })}
          </div>
