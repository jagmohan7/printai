          ...
          {industries.map((it, i) => {
            ...
            return (
              <MotionInView key={i} delay={0.08 * i}>
                <div className="group flex items-center gap-4 px-6 py-5 rounded-2xl border transition-all duration-300 cursor-default border-[#1E293B] bg-[#0F172A] hover:border-[#06b6d4]/40 hover:shadow-[0_0_22px_rgba(6,182,212,0.2)]">
                  <div className="w-11 h-11 rounded-xl flex items-center justify-center shrink-0 transition-colors duration-300 bg-[#0F172A] border border-[#1E293B] group-hover:bg-cyan-500/10 group-hover:border-cyan-500/20">
                    <Icon size={18} className="text-[#06b6d4] group-hover:text-[#22d3ee]" strokeWidth={1.5} />
                  </div>
                  <span className="text-white font-medium text-[15px]">{it.label}</span>
                </div>
              </MotionInView>
            );
          })}
