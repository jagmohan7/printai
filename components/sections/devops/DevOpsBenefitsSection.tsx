          ...
          {benefits.map((b, i) => {
            ...
            return (
              <MotionInView key={i} delay={0.09 * i}>
                <div className="group h-full p-7 rounded-2xl border transition-all duration-300 border-[var(--pa-line)] bg-[var(--pa-card)] hover:border-[#a78bfa]/30 hover:-translate-y-1 hover:shadow-[0_0_25px_rgba(139,92,246,0.2)]">
                  <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-5 transition-colors duration-300 bg-[var(--pa-card)] border border-[var(--pa-line)] group-hover:bg-violet-500/10 group-hover:border-violet-500/20">
                    <Icon size={20} className="text-[#06b6d4]" strokeWidth={1.5} />
                  </div>
                  <h3 className="pa-ink-text font-bold text-[16px] mb-2">{b.title}</h3>
                  <p className="pa-soft text-[14px] leading-relaxed">{b.description}</p>
                </div>
              </MotionInView>
            );
          })}
