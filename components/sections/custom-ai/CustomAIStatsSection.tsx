        ...
        <div className={`grid grid-cols-2 ${colsClass} gap-5`}>
          {stats.map((s, i) => (
            <MotionInView key={i} delay={0.12 * i}>
              <div className="ai-card group h-full flex flex-col items-center justify-center text-center py-10 px-5 rounded-2xl border border-white/[0.07] bg-[#12131f] hover:border-[#a78bfa]/40 hover:-translate-y-1 hover:shadow-[0_8px_36px_rgba(124,58,237,0.14)] transition-all duration-300">
                ...
                <span className="mt-3 text-white font-semibold text-[13px] leading-snug">{s.label}</span>
                {s.sub && <span className="mt-1.5 text-[#9ca3af] text-[12px]">{s.sub}</span>}
              </div>
            </MotionInView>
          ))}
        </div>
